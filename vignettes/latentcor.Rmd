...
**Input**: $F(r)=F(r, \Delta_{j}, \Delta_{k})$ - bridge function based on the type of variables $j$, $k$

   - Step 1. Calculate $\hat{\tau}_{jk}$ using (1).
   
   - Step 2. For binary/truncated variable $j$, set $\hat{\Delta}_{j}=\Phi^{-1}(\pi_{0j})$ with $\pi_{0j}=\sum_{i=1}^{n}I(x_{ij}=0)/n$. For ternary variable $j$, set $\hat{\Delta}=\Phi^{-1}(\pi_{0j}+\pi_{1j})$ with $\pi_{0j}=\sum_{i=1}^{n}I(x_{ij}=0)/n$ and $\pi_{1j}=\sum_{i=1}^{n}I(x_{ij}=1)/n$.
...
To avoid interpolation in areas with high approximation errors close to the boundary, we use hybrid scheme [@yoon2021fast]. The derivation of approximate bound for BC, BB, TC, TB, TT cases see [@yoon2021fast]. The derivation of approximate bound for NC, NB, NN cases see [@quan2018rank]. The derivation of approximate bound for NT case see Appendix.
