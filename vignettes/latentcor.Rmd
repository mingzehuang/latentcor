...
$$
\tilde{f}(x, y)=(1-\alpha)(1-\beta)f_{00}+(1-\alpha)\beta f_{01}+\alpha(1-\beta)f_{10}+\alpha\beta f_{11}
$$
...
\begin{align}
\tilde{f}(x, y, z)= & (1-\alpha)(1-\beta)(1-\gamma)f_{000}+(1-\alpha)(1-\beta)\gamma f_{001}+(1-\alpha)\beta(1-\gamma)f_{010} \nonumber\\
...
**Input**: Pre-computed values $F^{-1}(g)$ on a fixed grid $g\in\cal{G}$ based on the type of variables $j$ and $k$. For binary/continuous case, $g=(\tau_{jk}, \Delta{j})$; for binary/binary case, $g=(\tau_{jk}, \Delta{j}, \Delta{k})$; for truncated/continuous case, $g=(\tau_{jk}, \Delta_{j})$; for truncated/truncated case, $g=(\tau_{jk}, \Delta{j}, \Delta{k})$; for ternary/continuous case, $g=(\tau_{jk}, \Delta_{j}^{1}, \Delta_{j}^{2})$; for ternary/binary case, $g=(\tau_{jk}, \Delta_{j}^{1}, \Delta_{j}^{2}, \Delta_{k})$; for ternary/truncated case, $g=(\tau_{jk}, \Delta_{j}^{1}, \Delta_{j}^{2}, \Delta_{k})$; for ternay/ternary case, $g=(\tau_{jk}, \Delta_{j}^{1}, \Delta_{j}^{2}, \Delta_{k}^{1}, \Delta_{k}^{2})$.

  - Step 1 and Step 2 same as Algorithm 1.
  
  - Step 3. Set $\hat{r}_{jk}=\tilde{F}^{-1}(\hat{g})$, where $\tilde{F}^{-1}$ is the multilinear interpolation of $F^{-1}$ using $\cal{G}$.
...
\begin{align}
    \overline{\tau}_{BC}(\pi_{0j})=2\pi_{0j}(1-\pi_{0j})
\end{align}
\begin{align}
...
Note that $|\hat{\tau}|\le \bar{\tau}$, the grid does not need to cover the domain $\tau\in[-1, 1]$. Instead, we rescale $\tau$ as $\tilde{\tau}=\frac{\tau}{\bar{\tau}}$, then we have $\tilde{\tau}\in[-1, 1]$.

For ternary variable $j$, we know $\Delta_{j}^{2}>\Delta_{j}^{1}$ always holds since $\Delta_{j}^{1}=\Phi^{-1}(\pi_{0j})$ and $\Delta_{j}^{2}=\Phi^{-1}(\pi_{0j}+\pi_{1j})$, the grid should not cover the domain for the areas of $\Delta_{j}^{2}\ge\Delta_{j}^{1}$. Instead, we rescale $\Delta_{j}^{1}$ as $\tilde{\Delta}_{j}^{1}=\frac{\Delta_{j}^{1}}{\Delta_{j}^{2}}\in[0, 1]$, but keep $\Delta_{j}^{2}$ as $\tilde{\Delta}_{j}^{2}=\Delta_{j}^{2}\in[0, 1]$.

#### Algorithm 3 (Multi-linear interpolation with boundary method)
**Input**: Pre-computed values $F^{-1}(g)$ on a fixed grid $g\in\cal{G}$ based on the type of variables $j$ and $k$. For binary/continuous case, $g=(\tau_{jk}, \Delta{j})$; for binary/binary case, $g=(\tau_{jk}, \Delta{j}, \Delta{k})$; for truncated/continuous case, $g=(\tau_{jk}, \Delta_{j})$; for truncated/truncated case, $g=(\tau_{jk}, \Delta{j}, \Delta{k})$; for ternary/continuous case, $g=(\tau_{jk}, \Delta_{0j}, \Delta_{1j})$; for ternary/binary case, $g=(\tau_{jk}, \Delta_{0j}, \Delta_{1j}, \Delta_{k})$; for ternary/truncated case, $g=(\tau_{jk}, \Delta_{0j}, \Delta_{1j}, \Delta_{k})$; for ternary/ternary case, $g=(\tau_{jk}, \Delta_{1j}, \Delta_{2j}, \Delta_{1k}, \Delta_{2k})$.

  - Step 1 and Step2 same as Algorithm 1.
  
  - Step 3. If $|\hat{\tau_{jk}}|\le 0.9\cdot ABD$, apply multilinear interpolation Algorithm 2. otherwise, apply original Algorithm 1.
