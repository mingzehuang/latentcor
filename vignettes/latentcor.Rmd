...
$$
\widehat\sigma_{jk} = \arg\min_{\sigma} (F(\sigma) - \widehat \tau_{jk})^2.
$$
The parameter `tol` controls the desired accuracy of the minimizer and is passed to `optimize`:

```{r callR, warning = FALSE, message = F}
estimate = latentcor(X, types = c("con", "bin", "ter", "tru"), method = "original")
```
