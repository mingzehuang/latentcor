...
fromKtoR <- function(K, zratio, type, method, tol, ratio) {
  ...
  if (length(K) == 1){return(as.matrix(1))}
    ...
    hatRupper = R_sol(type1 = type, type2 = type, tau = c(K[upperR]), zratio1 = as.matrix(zratio1[upperR, ]), zratio2 = as.matrix(zratio2[upperR, ]), method = method, tol = tol, ratio = ratio)
...
fromKtoR_mixed <- function(K12, zratio1, zratio2, type1, type2, method, tol, ratio) {
  ...
  if ((type1 == "continuous" & type2 == "binary") | (type1 == "continuous" & type2 == "trunc") |
    ...
    hatR = R_sol(type1 = type2, type2 = type1, tau = c(t(K12)), zratio1 = zratio2, zratio2 = zratio1, method = method, tol = tol, ratio = ratio)
    return(matrix(hatR, d1, d2, byrow = TRUE))
  } else {
    hatR = R_sol(type1 = type1, type2 = type2, tau = c(K12), zratio1 = zratio1, zratio2 = zratio2, method = method, tol = tol, ratio = ratio)
