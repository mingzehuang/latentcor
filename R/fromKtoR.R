...
fromKtoR <- function(K, zratio, type, method, tol, ratio) {
  ...
  if (length(K) == 1){return(as.matrix(1))}
    ...
    hatRupper = R_sol(type1 = type, type2 = type, tau = c(K[upperR]), zratio1 = as.matrix(zratio1[upperR, ], nrow = sum(upperR)), zratio2 = as.matrix(zratio2[upperR, ], nrow = sum(upperR)), method = method, tol = tol, ratio = ratio)
